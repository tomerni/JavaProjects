...
public class BoopingSiteTest {
	...
	public void ratingSortTest() {
		...
		assertEquals("top rating failure", "baragarh villa", hotels[0].getPropertyName()); // top rating and
		// name
		assertEquals("last rating failure", "woodyvu mansari cottage",
					 hotels[hotels.length - 1].getPropertyName()); // last

		// same rating but different names
		assertEquals("same rating but different names", "baragarh villa", hotels[0].getPropertyName());
		assertEquals("same rating but different names", "bharhka countryside cottage resort",
					 hotels[1].getPropertyName());

		Hotel[] hotels1 = boop.getHotelsInCityByRating("jerusalem");
		assertEquals("invalid city failure", 0, hotels1.length); // returning empty array
	...
	public void sortProximityCityTest() {
		...
		// checks sorting
		assertEquals("closest failure", hotels[0].getPropertyName()); // closest
		assertEquals("farthest failure", "solang cottage",
					 hotels[hotels.length - 1].getPropertyName()); // farthest

		// same proximity (both 0.2907864282029195) but different POI
		assertEquals("same proximity (both 0.2907864282029195) but different POI", "apple inn cottage",
					 hotels[21].getPropertyName()); // 19 POI
		assertEquals("same proximity (both 0.2907864282029195) but different POI", "sunrise cottage",
		...
		assertEquals("invalid coordinates", 0, hotels2.length); // returning empty array
		Hotel[] hotels3 = boop.getHotelsInCityByProximity("manali", 32, -200);
		assertEquals("invalid coordinates", 0, hotels3.length); // returning empty array

		Hotel[] hotels4 = boop.getHotelsInCityByRating(null); // null city
		assertEquals("null city", 0, hotels4.length); // returning empty array
	}
	...
	public void ratingSortTestFull() {
		...
		assertEquals("invalid city failure", 0, hotels2.length); // returning empty array
	}
	...
	public void sortProximityCityTestFull() {
		...
		assertEquals("closest failure", "tivoli grand resort hotel", hotels[0].getPropertyName()); // closest
		assertEquals("farthest failure", "v resorts farm stay",
					 hotels[hotels.length - 1].getPropertyName()); // farthest
	...
	public void sortProximityTestFull() {
		...
		assertEquals("closest failure", "upadhyay cottages", hotels[0].getPropertyName()); // closest
		assertEquals("farthest failure", "tsg aura", hotels[hotels.length - 1].getPropertyName()); //
		// farthest

		// same proximity (both 0.2997962565585185) but different POI
		assertEquals("same proximity (both 0.2997962565585185) but different POI", "city heart hotel",
					 hotels[41].getPropertyName()); // 1 POI
		assertEquals("same proximity (both 0.2997962565585185) but different POI", "hotel shree vinayak",
